...
function Showtimes() {
    ...
    // Fetch movie details from API based on movieId
    useEffect(() => {
        const fetchMovieDetails = async () => {
            try {
                const id = movieId || (state?.movie?.movieId || "06f99c56-f972-4659-a13c-7b4758d44d6e");
                const response = await fetch(`http://localhost:5229/api/movie/getMovieDetail/${id}`);
                if (!response.ok) {
                ...
                setMovieInfo({
                    ...
                    movieMinimumAge: Object.values(data.movieMinimumAge)[0] as string,
                    ...
                    movieLanguage: Object.values(data.movieLanguage)[0] as string,
    ...
    const food = [
        { key: "Popcorn", img: "https://api-website.cinestar.com.vn/media/.thumbswysiwyg/pictures/HinhQuayconnew/baprang.jpg?rand=1719572360", label: "Bắp rang", price: 50000 },
    ];
    ...
    return (
        <div className="flex flex-col min-h-screen bg-fixed w-full bg-cover bg-center top-0" style={{ backgroundImage: "url('https://images8.alphacoders.com/136/thumb-1920-1368754.jpeg')" }}>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 mb-6 justify-center items-center">
                        ...
                        <div>
                            ...
                            <ul className="text-white mb-4">
                                <li><span className="text-yellow-400 font-bold">Thể loại:</span> <span className="pl-6">{movieInfo.movieGenre.join(", ")}</span></li>
                                <li><span className="text-yellow-400 font-bold">Thời lượng:</span> <span className="pl-6">{movieInfo.movieDuration}'</span></li>
                                <li><span className="text-yellow-400 font-bold">Ngôn ngữ:</span> <span className="pl-6">{movieInfo.movieLanguage}</span></li>
                                <li><span className="text-yellow-400 font-bold">Đạo diễn:</span> <span className="pl-6">{movieInfo.movieDirector}</span></li>
                                <li><span className="text-yellow-400 font-bold">Diễn viên:</span> <span className="pl-6">{movieInfo.movieActor}</span></li>
